...
echo $chrom";"$pos";"$motif";"$thresh";"$freqEUR";"$freqAFR";"$freqEAS";"$freqAMR";"$freqSAS | sed 's/;/\t/g'
exit 0
(base) [mgymrek@snorlax callset-qc]$ git add^Cenerate_afreqs.sh
(base) [mgymrek@snorlax callset-qc]$ cat ge
cat: ge: No such file or directory
(base) [mgymrek@snorlax callset-qc]$ cat generate_afreqs.sh
#!/bin/bash
